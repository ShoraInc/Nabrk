...
const updateEventTranslation = async (req, res) => {
  try {
    ...
    console.log("Updating event translation:", {
      id,
      lang,
      name,
      description,
    ...
    // Handle name - always update/create, even if empty
    const existingNameTranslation = await TextTranslations.findOne({
      where: { textId: event.nameTextId, languageCode: lang },
    });
    ...
    // Handle description - always update/create, even if empty
    const existingDescriptionTranslation = await TextTranslations.findOne({
      where: { textId: event.descriptionTextId, languageCode: lang },
    });

    if (existingDescriptionTranslation) {
      await TextTranslations.update(
        { textValue: description || "" },
        { where: { textId: event.descriptionTextId, languageCode: lang } }
      );
    ...
    // Handle place - always update/create, even if empty
    const existingPlaceTranslation = await TextTranslations.findOne({
      where: { textId: event.placeTextId, languageCode: lang },
    });

    if (existingPlaceTranslation) {
      await TextTranslations.update(
        { textValue: place || "" },
        { where: { textId: event.placeTextId, languageCode: lang } }
      );
    ...
    console.log(`Event translation updated successfully for ${lang}`);
...
const activateEvent = async (req, res) => {
  try {
    ...
    await Events.update({ isActive: true }, { where: { id } });
...
const deactivateEvent = async (req, res) => {
  try {
    ...
    await Events.update({ isActive: false }, { where: { id } });
...
const deleteEvent = async (req, res) => {
  try {
    ...
    const textIdsToDelete = [
      event.nameTextId,
      event.descriptionTextId,
      event.placeTextId,
    ];
