        ...
        const newsWithTranslations = await Promise.all(
            news.rows.map(async (item) => {
                const [title, shortDescription] = await Promise.all([
                    ...
                    item.shortDescriptionTextId ? TextTranslations.findOne({
                        where: { textId: item.shortDescriptionTextId, languageCode: lang }
                    }) : null
                ]);
...
const getNewsById = async (req, res) => {
    try {
        ...
        const [title, shortDescription, content] = await Promise.all([
            ...
            news.shortDescriptionTextId ? TextTranslations.findOne({
            ...
            if (!news.shortDescriptionTextId) {
                ...
                for (const langRecord of existingLanguages) {
                    await TextTranslations.create({
                        ...
                        textValue: langRecord.languageCode === lang ? shortDescription : '' // Only current language gets value, others get empty string
                    });
                }
            } else {
