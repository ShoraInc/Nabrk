...
const AddMenuItemModalWithPageSearch = ({ parentItem, onSuccess, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim().length >= 2) {
      searchTimeoutRef.current = setTimeout(async () => {
        ...
        try {
          const results = await menuApi.searchPages(searchQuery);
          setSearchResults(results);
          setShowSearchResults(true);
        } catch (err) {
          console.error("Ошибка поиска страниц:", err);
          setSearchResults([]);
        } finally {
            ...
            <div className="mb-4">
              ...
              <div className="relative" ref={searchInputRef}>
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="Введите название страницы для поиска..."
                  disabled={loading}
                  required
                />
                {searchLoading && (
              ...
              {selectedPage && (
                <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-md">
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="text-sm font-medium text-green-800">{selectedPage.title}</div>
                      <div className="text-xs text-green-600">/{selectedPage.slug}</div>
                    </div>
                  </div>
                </div>
              )}
              ...
              <p className="mt-1 text-xs text-gray-500">
                Начните вводить название страницы для поиска
              </p>
            </div>
          )}
...
export default AddMenuItemModalWithPageSearch;
