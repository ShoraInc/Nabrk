  ...
  const loadEvents = async () => {
    try {
      ...
      if (response.events && response.events.length > 0) {
        ...
        const upcomingDates = EventsApi.getUpcomingDatesWithEvents(groupedEvents, 7);
        
        setEventsData(upcomingDates);
        
        // Устанавливаем активную дату на первый день с событиями
        const firstDateWithEvents = upcomingDates.findIndex(date => date.hasEvents);
        if (firstDateWithEvents !== -1) {
          setActiveDate(firstDateWithEvents);
        }
      } else {
        // Если нет событий, показываем следующие 7 дней
    ...
    <div className="events-container">
      ...
      <div style={{
        ...
        <p style={{
          ...
          opacity: 0.8,
          margin: '0 0 16px 0'
        }}>Іс-шараларды жүктеу мүмкін болмады</p>
        <button 
          onClick={loadEvents}
          style={{
            padding: '12px 24px',
  ...
  const EmptyDateState = () => (
    <div style={{
      ...
      <h3 style={{
        ...
        fontWeight: '300',
        margin: '0 0 4px 0'
      }}>Бұл күні іс-шаралар жоқ</h3>
      <p style={{
        fontSize: '12px',
        opacity: 0.7,
        margin: 0
      }}>Басқа күндерді таңдап көріңіз</p>
    </div>
  );
